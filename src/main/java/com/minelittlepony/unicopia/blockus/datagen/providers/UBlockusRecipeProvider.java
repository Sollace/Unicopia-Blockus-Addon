...
public class UBlockusRecipeProvider extends BlockusRecipeProvider {
    ...
    public void generate(RecipeExporter exporter) {
        UBlockusBlocks.woodsets().forEach(woodset -> {
            ...
            createTimberFramesRecipes(exporter, woodset.planks(), woodset.timberFrames().get(0), woodset.timberFrames().get(1), woodset.timberFrames().get(2));
        });
    ...
    private void offerBSSRecipes(List<Block> trio, RecipeExporter exporter) {
        RecipeProvider.offerSlabRecipe(exporter, RecipeCategory.BUILDING_BLOCKS, trio.get(1), trio.get(0));
        offerStairsRecipe(exporter, trio.get(2), trio.get(0));
    }
